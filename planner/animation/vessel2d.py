...
with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
    ...
    while True:
        ...
        plt.plot(besty[1:], bestx[1:], "-oc", markersize=3, alpha=0.4)

        plt.axis('equal')
        plt.xlim(doubles_sequence[1] - area, doubles_sequence[1] + area)
        plt.ylim(doubles_sequence[0] - area, doubles_sequence[0] + area)
