...
//-----------------------------------------------------------------------------
tQCustomFrame::tQCustomFrame(tTargetLocation* pTargets, unsigned maxTargets,
                             QWidget* p, QImage* pImage,
...
void tQCustomFrame::contextMenuEvent(QContextMenuEvent* pEvent) {
  if (m_pImage) {
    ...
    for (unsigned i = 0; i < dimof(targetSizes); ++i) {
      ...
      if (m_TargetRadius == targetSizes[i].radius) pAction->setChecked(true);
    }
