...
class NAVICO_SDK_EXPORT tTargetTrackingClient {
 public:
  ...
  //-----------------------------------------------------------------------------------
  //! Connects to the Radar with the specified 'serialNumber'. Any existing
  //! connection will be automatically disconnected first. The radar must be
  //! unlocked before this operation can succeed (see MultiRadarClient.h). This
  //! will also create the threads required to listen on the connections made.
  ...
  //! \return  0 if successful, or one of the eErrors (ClientErrors.h) enums if
  //! failed
  //-----------------------------------------------------------------------------------
  ...
  //! \param range_m The range where the radar should look to start tracking, in
  //! metres. \param bearing_deg The bearing where the radar should look to
  //! start tracking, in
  //!     degrees.
  //! \param bearingType If eAbsolute, the bearing will be interpreted as true
  //! north
  //!     bearing, if eRelative the bearing will be interpreted as relative to
  //!     the heading of the vessel.
  //! \return  True if the operation was successful
  //-----------------------------------------------------------------------------------
  bool Acquire(uint32_t id, uint32_t range_m, uint16_t bearing_deg,
               eBearingType bearingType);

  //-----------------------------------------------------------------------------------
  //! Cancel a tracked target. Also required to release server-ID's of targets
  //! that have been lost. \param serverID  The id of the target to be
  //! cancelled. \return  True if the operation was successful
  //-----------------------------------------------------------------------------------
  ...
  //! \param speedType Type of speed supplied: eSpeedOverGround (1), eWaterSpeed
  //! (2) is the water speed. Only one kind should be provided
  //! \param speed_dmps Speed magnitude expressed in 10ths of a metre per second
  //! (ie. deci-metres per second)
  //! \param directionType Type of direction supplied: eCoureOverGround (1);
  //! eHeadingMagnetic (2); or eHeadingTrue (3)
  //! \param direction_deg Direction of the vessel expressed in degrees.
  //! \return True if the operation was successful
  //-----------------------------------------------------------------------------------
  bool SetBoatSpeed(eSpeedType speedType, uint32_t speed_dmps,
                    eDirectionType directionType, uint32_t direction_deg);
  ...
  //-----------------------------------------------------------------------------------
  //! Sets the danger zone distance. Used in connection with the danger zone
  //! time (see SetDangerTime) to provide early warning of a possible collision
  //! with a target. \param range_m Danger distance in metres \return True if
  //! the operation was successful
  //-----------------------------------------------------------------------------------
  bool SetDangerDistance(uint32_t range_m);

  //-----------------------------------------------------------------------------------
  //! Sets the danger zone time. Used in connection with the danger zone
  //! distance (see SetDangerDistance) to provide early warning of a possible
  //! collision with a target. \param time_sec Danger time in seconds \return
