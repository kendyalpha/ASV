...
void tTabGuardZone::SetGuardZone(int zone) {
  if (m_pImageClient) {
    ...
    m_pImageClient->SetGuardZoneSetup(zone, startRange_m, endRange_m,
                                      bearing_deg, width_deg);

    // Update the guard zone overlay in GUI
    m_OverlayManager.SetGuardZone(zone, enabled, startRange_m, endRange_m,
                                  bearing_deg, width_deg);
  }
}
...
void tTabGuardZone::OnSetupChanged(
  ...
  m_Ui.editGuardSensitivity->setText(
      QString::number(pSetup->guardzones.sensitivity));

  bool gz1Enabled = pSetup->guardzones.active[eGuardZone1];
  m_Ui.checkGuard1Enabled->setChecked(gz1Enabled);
  m_Ui.editGuard1RangeStart->setText(
      QString::number(pSetup->guardzones.zone[eGuardZone1].rangeStart_m));
  m_Ui.editGuard1RangeEnd->setText(
      QString::number(pSetup->guardzones.zone[eGuardZone1].rangeEnd_m));
  m_Ui.editGuard1Bearing->setText(QString::number(
      pSetup->guardzones.zone[eGuardZone1].azimuth_ddeg / 10.0, 'f', 1));
  m_Ui.editGuard1Width->setText(QString::number(
      pSetup->guardzones.zone[eGuardZone1].width_ddeg / 10.0, 'f', 1));
  m_Ui.editGuard1AlarmType->setText(
      ToItemName(pSetup->guardzones.alarmType[eGuardZone1].alarmType,
                 m_Ui.comboGuard1AlarmType));
  ...
  m_Ui.editGuard2RangeStart->setText(
      QString::number(pSetup->guardzones.zone[eGuardZone2].rangeStart_m));
  m_Ui.editGuard2RangeEnd->setText(
      QString::number(pSetup->guardzones.zone[eGuardZone2].rangeEnd_m));
  m_Ui.editGuard2Bearing->setText(QString::number(
      pSetup->guardzones.zone[eGuardZone2].azimuth_ddeg / 10.0, 'f', 1));
  m_Ui.editGuard2Width->setText(QString::number(
      pSetup->guardzones.zone[eGuardZone2].width_ddeg / 10.0, 'f', 1));
  m_Ui.editGuard2AlarmType->setText(
      ToItemName(pSetup->guardzones.alarmType[eGuardZone2].alarmType,
                 m_Ui.comboGuard2AlarmType));
  m_OverlayManager.SetGuardZone(
  ...
  if (m_UpdateOnSetup) {
    ...
    SetManualValue(m_Ui.spinGuard1RangeStart,
                   pSetup->guardzones.zone[eGuardZone1].rangeStart_m);
    SetManualValue(m_Ui.spinGuard1RangeEnd,
                   pSetup->guardzones.zone[eGuardZone1].rangeEnd_m);
    SetManualValue(m_Ui.spinGuard1Bearing,
                   pSetup->guardzones.zone[eGuardZone1].azimuth_ddeg / 10.0);
    SetManualValue(m_Ui.spinGuard1Width,
                   pSetup->guardzones.zone[eGuardZone1].width_ddeg / 10.0);
    SetManualValue(m_Ui.comboGuard1AlarmType,
                   pSetup->guardzones.alarmType[eGuardZone1].alarmType);

    SetManualValue(m_Ui.spinGuard2RangeStart,
                   pSetup->guardzones.zone[eGuardZone2].rangeStart_m);
    SetManualValue(m_Ui.spinGuard2RangeEnd,
                   pSetup->guardzones.zone[eGuardZone2].rangeEnd_m);
    SetManualValue(m_Ui.spinGuard2Bearing,
                   pSetup->guardzones.zone[eGuardZone2].azimuth_ddeg / 10.0);
    SetManualValue(m_Ui.spinGuard2Width,
                   pSetup->guardzones.zone[eGuardZone2].width_ddeg / 10.0);
    SetManualValue(m_Ui.comboGuard2AlarmType,
...
void tTabGuardZone::OnGuardZoneAlarmChanged(
  ...
  switch (pAlarm->state) {
    ...
    default: { state = QString("Unknown %0").arg(pAlarm->state); }
  }
