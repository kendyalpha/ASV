...
tTabSectorBlanking::tTabSectorBlanking(Ui::GUIDemoClass& ui, QObject* pParent,
    ...
    : tTabBase(ui, pParent, tab), m_OverlayManager(overlayManager) {
...
void tTabSectorBlanking::UpdateSector(unsigned sector, bool enabled,
                                      double start, double end) {
  m_pImageClient->SetSectorBlankingSetup(sector, enabled, ToDeciDegrees(start),
                                         ToDeciDegrees(end));
  m_OverlayManager.SetBlankingSector(sector, enabled, ToDegrees(start),
                                     ToDegrees(end));
}

void tTabSectorBlanking::Sector0Start_valueChanged(double value) {
  UpdateSector(0, m_Ui.checkSector0Enabled->checkState() == Qt::Checked, value,
               m_Ui.spinSector0End->value());
}

void tTabSectorBlanking::Sector0End_valueChanged(double value) {
  UpdateSector(0, m_Ui.checkSector0Enabled->checkState() == Qt::Checked,
               m_Ui.spinSector0Start->value(), value);
}

void tTabSectorBlanking::Sector1Start_valueChanged(double value) {
  UpdateSector(1, m_Ui.checkSector1Enabled->checkState() == Qt::Checked, value,
               m_Ui.spinSector1End->value());
}

void tTabSectorBlanking::Sector1End_valueChanged(double value) {
  UpdateSector(1, m_Ui.checkSector1Enabled->checkState() == Qt::Checked,
               m_Ui.spinSector1Start->value(), value);
}

void tTabSectorBlanking::Sector2Start_valueChanged(double value) {
  UpdateSector(2, m_Ui.checkSector2Enabled->checkState() == Qt::Checked, value,
               m_Ui.spinSector2End->value());
}

void tTabSectorBlanking::Sector2End_valueChanged(double value) {
  UpdateSector(2, m_Ui.checkSector2Enabled->checkState() == Qt::Checked,
               m_Ui.spinSector2Start->value(), value);
}

void tTabSectorBlanking::Sector3Start_valueChanged(double value) {
  UpdateSector(3, m_Ui.checkSector3Enabled->checkState() == Qt::Checked, value,
               m_Ui.spinSector3End->value());
}

void tTabSectorBlanking::Sector3End_valueChanged(double value) {
  UpdateSector(3, m_Ui.checkSector3Enabled->checkState() == Qt::Checked,
               m_Ui.spinSector3Start->value(), value);
}

void tTabSectorBlanking::Sector4Start_valueChanged(double value) {
  UpdateSector(4, m_Ui.checkSector4Enabled->checkState() == Qt::Checked, value,
               m_Ui.spinSector4End->value());
}

void tTabSectorBlanking::Sector4End_valueChanged(double value) {
  UpdateSector(4, m_Ui.checkSector4Enabled->checkState() == Qt::Checked,
               m_Ui.spinSector4Start->value(), value);
}

void tTabSectorBlanking::Sector5Start_valueChanged(double value) {
  UpdateSector(5, m_Ui.checkSector5Enabled->checkState() == Qt::Checked, value,
               m_Ui.spinSector5End->value());
}

void tTabSectorBlanking::Sector5End_valueChanged(double value) {
  UpdateSector(5, m_Ui.checkSector5Enabled->checkState() == Qt::Checked,
               m_Ui.spinSector5Start->value(), value);
}

//-----------------------------------------------------------------------------
void tTabSectorBlanking::Sector0Enabled_clicked(bool checked) {
  UpdateSector(0, checked, m_Ui.spinSector0Start->value(),
               m_Ui.spinSector0End->value());
}

void tTabSectorBlanking::Sector1Enabled_clicked(bool checked) {
  UpdateSector(1, checked, m_Ui.spinSector1Start->value(),
               m_Ui.spinSector1End->value());
}

void tTabSectorBlanking::Sector2Enabled_clicked(bool checked) {
  UpdateSector(2, checked, m_Ui.spinSector2Start->value(),
               m_Ui.spinSector2End->value());
}

void tTabSectorBlanking::Sector3Enabled_clicked(bool checked) {
  UpdateSector(3, checked, m_Ui.spinSector3Start->value(),
               m_Ui.spinSector3End->value());
}

void tTabSectorBlanking::Sector4Enabled_clicked(bool checked) {
  UpdateSector(4, checked, m_Ui.spinSector4Start->value(),
               m_Ui.spinSector4End->value());
}

void tTabSectorBlanking::Sector5Enabled_clicked(bool checked) {
  UpdateSector(5, checked, m_Ui.spinSector5Start->value(),
               m_Ui.spinSector5End->value());
}
...
// State Changes
//-----------------------------------------------------------------------------
void tTabSectorBlanking::OnConfigurationChanged(
  ...
  if (m_UpdateOnConfiguration) {
    ...
    uint16_t startBearing_deg =
        ToDegrees(pConfig->blankSectors[0].sectorStart_ddeg / 10.0);
    uint16_t endBearing_deg =
        ToDegrees(pConfig->blankSectors[0].sectorEnd_ddeg / 10.0);
    ...
    m_OverlayManager.SetBlankingSector(0, enabled, startBearing_deg,
                                       endBearing_deg);

    enabled = pConfig->blankSectors[1].state;
    startBearing_deg =
        ToDegrees(pConfig->blankSectors[1].sectorStart_ddeg / 10.0);
    ...
    m_OverlayManager.SetBlankingSector(1, enabled, startBearing_deg,
                                       endBearing_deg);

    enabled = pConfig->blankSectors[2].state;
    startBearing_deg =
        ToDegrees(pConfig->blankSectors[2].sectorStart_ddeg / 10.0);
    ...
    m_OverlayManager.SetBlankingSector(2, enabled, startBearing_deg,
                                       endBearing_deg);

    enabled = pConfig->blankSectors[3].state;
    startBearing_deg =
        ToDegrees(pConfig->blankSectors[3].sectorStart_ddeg / 10.0);
    ...
    if (Navico::Protocol::NRP::cMaxBlankSectors > 4) {
      ...
      endBearing_deg =
          ToDegrees(pConfig->blankSectors[4].sectorEnd_ddeg / 10.0);
      ...
      m_OverlayManager.SetBlankingSector(4, enabled, startBearing_deg,
                                         endBearing_deg);
    }
    ...
    if (Navico::Protocol::NRP::cMaxBlankSectors > 5) {
      ...
      endBearing_deg =
          ToDegrees(pConfig->blankSectors[5].sectorEnd_ddeg / 10.0);
      ...
      m_OverlayManager.SetBlankingSector(5, enabled, startBearing_deg,
                                         endBearing_deg);
    }
  }
}
