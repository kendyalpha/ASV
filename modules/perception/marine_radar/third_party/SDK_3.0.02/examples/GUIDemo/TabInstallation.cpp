...
//-----------------------------------------------------------------------------
tTabInstallation::tTabInstallation(Ui::GUIDemoClass& ui, QObject* pParent,
                                   QWidget& tab)
    : tTabBase(ui, pParent, tab), m_antennaTypeIndex(0) {
...
void tTabInstallation::AntennaXOffset_valueChanged(double value) {
  m_pImageClient->SetAntennaOffsets(int32_t(value * 1000.0),
                                    m_Ui.spinAntennaYOffset->value() * 1000.0);
}

//-----------------------------------------------------------------------------
void tTabInstallation::AntennaYOffset_valueChanged(double value) {
  m_pImageClient->SetAntennaOffsets(m_Ui.spinAntennaXOffset->value() * 1000.0,
                                    int32_t(value * 1000.0));
}
...
void tTabInstallation::OnConfigurationChanged(
  ...
  m_Ui.editAntennaHeight->setText(
      QString::number(pConfig->antennaHeight_mm / 1000.0, 'f', 3));
  m_Ui.editZeroRange->setText(
      QString::number(pConfig->zeroRange_mm / 1000.0, 'f', 3));
  m_Ui.editZeroBearing->setText(
      QString::number(pConfig->zeroBearing_ddeg / 10.0, 'f', 1));
  m_Ui.editAntennaXOffset->setText(
      QString::number(pConfig->antennaOffsetX_mm.Get() / 1000.0, 'f', 1));
  m_Ui.editAntennaYOffset->setText(
      QString::number(pConfig->antennaOffsetY_mm.Get() / 1000.0, 'f', 1));

  m_antennaTypeIndex = pConfig->antennaType;
  if (m_antennaTypeIndex >= 0 &&
      m_antennaTypeIndex < m_Ui.comboAntennaType->count()) {
  ...
  if (m_UpdateOnConfiguration) {
    ...
    SetManualValue(m_Ui.spinAntennaXOffset,
                   pConfig->antennaOffsetX_mm.Get() / 1000.0);
    SetManualValue(m_Ui.spinAntennaYOffset,
                   pConfig->antennaOffsetY_mm.Get() / 1000.0);
  }
}
