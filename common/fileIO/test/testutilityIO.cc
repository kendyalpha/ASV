...
int main() {
  ...
  std::vector<double> mat2vec = ASV::convertEigenMat2stdvector(mat);
  ...
  std::vector<double> vec2vec = ASV::convertEigenVec2stdvector(Vec);
  ...
  Eigen::MatrixXd vec2mat = ASV::convertstdvector2EigenMat(mat2vec, 3, 4);
  std::cout << vec2mat << std::endl;

  // unit test for writing Eigen matrix to csv file
  ASV::write2csvfile("../data/csvfile.csv", vec2mat);
}