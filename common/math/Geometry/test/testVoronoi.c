...
static void plot(int x, int y, unsigned char* image, int width, int height,
...
static void draw_line(int x0, int y0, int x1, int y1, unsigned char* image,
...
static inline int orient2d(const jcv_point* a, const jcv_point* b,
                           const jcv_point* c) {
...
static inline int max3(int a, int b, int c) { return max2(a, max2(b, c)); }

static void draw_triangle(const jcv_point* v0, const jcv_point* v1,
...
static void Usage() {
  ...
  printf(
      "\t-i <inputfile>\t\tA list of 2-tuples (float, float) representing 2-d "
      "coordinates\n");
