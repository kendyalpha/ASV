...
## 参数 ##
DBNAME = 'Mon Dec 23 15:57:19 2019.db'
num_thruster = 2
ACTUATOR = 'underactuated'  # underactuated
time_stamp_select = np.array([90, 130])
...
plt.suptitle("time series of marine state", fontsize=12)
plt.subplot(3, 1, 1)
plt.plot(estimatordata['DATETIME'], estimatordata['curvature'], '-r', lw=2)
plt.ylabel('curvature')

plt.subplot(3, 1, 2)
plt.plot(estimatordata['DATETIME'], estimatordata['speed'], '-r', lw=2)
plt.ylabel('speed (m/s)')

plt.subplot(3, 1, 3)
plt.plot(estimatordata['DATETIME'], estimatordata['dspeed'], '-r', lw=2)
plt.ylabel('dspeed (m/s^2)')
